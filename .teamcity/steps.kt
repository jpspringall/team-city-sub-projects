    ...
    fun BuildType.runMakeTest(
        ...
        steps {
            ...
            exec {
                ...
                arguments = """-i ""%env.TEAMCITY_VERSION%"" -s ""%env.sonar_server%"" -u ""%env.sonar_user%"" -p ""%env.sonar_password%"" -c ""%build.counter%"" -r ""%teamcity.pullRequest.number%"" -n ""%build.number%"""""
                formatStderrAsError = true
            }
        }
    }
